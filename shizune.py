...
def manage_content_cache():
    ...
    if len(sent_content_ids) > MAX_CONTENT_CACHE:
        ...
        logging.info(f"Content cache cleaned, now has {len(sent_content_ids)} items")
...
ANIME_COMMANDS = {
    ...
    "kushina": {
        ...
        "tags": ["kushina_uzumaki", "uzumaki_kushina", "kushina", "red_hair", "nine_tails_jinchuriki"]
    },
    "hinata": {
        "title": "Hinata Hyuga",
        "tags": ["hinata_hyuga", "hyuga_hinata", "hinata_(naruto)", "hinata", "byakugan", "lavender_eyes"]
    },
    "sakura": {
        "title": "Sakura Haruno",
        "tags": ["sakura_haruno", "haruno_sakura", "sakura_(naruto)", "sakura", "medical_ninja", "pink_hair"]
    },
    "temari": {
        "title": "Temari",
        "tags": ["temari_(naruto)", "temari", "sand_village", "wind_style", "blonde_hair", "fan_weapon"]
    },
    "konan": {
        "title": "Konan",
        "tags": ["konan", "paper_jutsu", "blue_hair", "akatsuki_member", "angel"]
    },
    "ino": {
        "title": "Ino Yamanaka",
        "tags": ["ino_yamanaka", "yamanaka_ino", "ino", "blonde_hair", "mind_transfer"]
    },
    "shizune": {
        "title": "Shizune",
        "tags": ["shizune", "medical_ninja", "black_hair", "tsunade_assistant", "leaf_village"]
    },
    "tsunade": {
        "title": "Tsunade",
        "tags": ["tsunade", "hokage", "blonde_hair", "large_breasts", "medical_ninja", "legendary_sannin"]
    },
    "sarada": {
        "title": "Sarada Uchiha",
        "tags": ["sarada_uchiha"]
...
async def fetch_rule34_media(anime_name: str, media_type: str = "image", user_id: int = 0, max_retries: int = 5):
    ...
    if not anime_data:
        logging.error(f"Anime {anime_name} not found in database")
        return None
    ...
    for retry in range(max_retries):
        try:
            ...
            logging.info(f"Attempt {retry + 1}: Searching Rule34 for {anime_name} with tags: {selected_tags}, page: {page_offset}")
            
            # Check rate limit before making API request
            if not check_rate_limit():
                logging.warning("Rate limit reached, waiting 10 seconds...")
                    ...
                    if response.status == 200:
                        ...
                        try:
                            ...
                            if posts:
                                ...
                                logging.info(f"Found fresh {anime_name} content: score {selected['score']}, ID: {selected['id']}")
                                return selected
                        except ET.ParseError as e:
                            logging.warning(f"XML parse error on attempt {retry + 1}: {e}")
                            continue
                    
        except Exception as e:
            logging.warning(f"Attempt {retry + 1} failed for {anime_name}: {e}")
            continue
    
    # Advanced fallback system with broader searches
    logging.info(f"Trying advanced fallback for {anime_name}")
                    ...
                    if response.status == 200:
                        ...
                        try:
                            ...
                            if posts:
                                ...
                                logging.info(f"Found fallback content for {anime_name} with tag {tag}, ID: {selected['id']}")
                                return selected
                        except ET.ParseError as e:
                            logging.warning(f"Fallback XML parse error for tag {tag}: {e}")
                            continue
        except Exception as e:
            logging.warning(f"Fallback failed for tag {tag}: {e}")
            continue
    
    logging.error(f"All attempts failed for {anime_name}")
    return None
...
async def send_media_selection(anime_name: str, chat_id: int):
    ...
    logging.info(f"Sending media selection for {title}")
    ...
    try:
        ...
        logging.error(f"Media selection error: {e}")
        return None
...
async def send_random_media(chat_id: int, message_id: int | None = None, edit_mode: bool = False, media_type: str = "image", page: int = 1):
    ...
    try:
        ...
        if edit_mode and message_id:
            ...
            logging.info(f"Successfully loaded random {media_type}")
        else:
            if media_type == "video":
            ...
            logging.info(f"Successfully sent random {media_type}")
            
        return True
    except Exception as e:
        logging.error(f"Random media send error: {e}")
        return None
...
async def send_search_media(search_query: str, chat_id: int, message_id: int | None = None, edit_mode: bool = False, media_type: str = "image", page: int = 1):
    ...
    try:
        ...
        if edit_mode and message_id:
            ...
            logging.info(f"Successfully loaded search {media_type} for '{search_query}'")
        else:
            if media_type == "video":
                await bot.send_video(
                    chat_id=chat_id,
            ...
            logging.info(f"Successfully sent search {media_type} for '{search_query}'")
            
        return True
    except Exception as e:
        logging.error(f"Search media send error: {e}")
        return None
...
async def send_anime_media(anime_name: str, chat_id: int, message_id: int | None = None, edit_mode: bool = False, media_type: str = "image", page: int = 1):
    ...
    logging.info(f"Fetching {title} {media_type} content using API")
    ...
    for attempt in range(15):
        ...
        if attempt >= 10 and media_type in ["video", "gif"]:
            logging.info(f"No {media_type} found for {anime_name}, falling back to images")
            post = await fetch_rule34_media(anime_name, "image", chat_id)
            if post:
                break
    
    if not post:
        logging.error(f"Failed to fetch any media for {anime_name}")
        return None
    ...
    try:
        ...
        logging.info(f"Sending {media_type} for {anime_name}: {post['url'][-50:]}")
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        if posts:
                            ...
                            selected = random.choice(top_posts)
                            logging.info(f"Found random content: score {selected['score']}")
                            return selected
                    except ET.ParseError as e:
                        logging.warning(f"Random content XML parse error: {e}")
    except Exception as e:
        logging.error(f"Random content fetch error: {e}")
...
async def search_rule34_live(search_query: str, media_type: str = "image"):
    ...
    try:
        ...
        logging.info(f"Live searching Rule34 for: '{formatted_query}' (original: '{clean_query}')")
                ...
                if response.status == 200:
                    ...
                    try:
                        ...
                        if posts:
                            ...
                            top_posts = posts[:50]  # Larger pool for more variety
                            selected = random.choice(top_posts)
                            logging.info(f"Found search result for '{search_query}': score {selected['score']}")
                            return selected
                    except ET.ParseError as e:
...
def create_random_selection_keyboard():
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="🖼️ Images", callback_data="select_image_random")
        ],
        [
...
def create_search_selection_keyboard(search_query: str):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="🖼️ Images", callback_data=f"select_image_{encoded_query}")
        ],
        [
...
I'm <b>Shizune</b> and I'm here to make your day better! 🌸
...
async def cmd_help(msg: Message):
    ...
    short_help_text = f"""
💝 <b>Shizune's Guide - <a href="tg://user?id={user_id}">{user_name}</a></b> 💝
...
async def send_random_selection(chat_id: int):
    ...
    logging.info("Sending random media selection")
    ...
    try:
        ...
        logging.info("Random media selection sent successfully")
        return True
    except Exception as e:
        logging.error(f"Random selection send error: {e}")
        return None

@dp.message(Command("random"))
async def cmd_random(msg: Message):
    """Handle random content command"""
    logging.info("Random command requested")
    
    try:
        await send_random_selection(msg.chat.id)
    except Exception as e:
        logging.error(f"Random command error: {e}")
...
"""
    ...
    try:
        ...
        logging.info(f"Live search selection sent for: {search_text}")
    except Exception as e:
        logging.error(f"Live search send error: {e}")
...
async def handle_callbacks(callback: CallbackQuery):
    ...
    if callback.data == "expand_help" or callback.data.startswith("help_page_"):
        ...
        if page == 1:
            help_text = f"""
💝 <b>Shizune's Complete Guide - <a href="tg://user?id={user_id}">{user_name}</a></b> 💝
...
"""
    ...
    if action == "select":
        ...
        logging.info(f"Media type {media_type} selected for: {target}")
        ...
        if target == "random":
            ...
            try:
                ...
                logging.error(f"Random selection error: {e}")
            ...
            try:
                ...
                logging.info(f"Successfully loaded {media_type} for {target}")
            except Exception as e:
                logging.error(f"Anime selection error: {e}")
            ...
            try:
                ...
                logging.error(f"Search selection error: {e}")
    ...
    # Handle navigation buttons (update, next)
        ...
        if action == "update":
            logging.info(f"Update button pressed for: {anime_name} ({media_type}, page {page})")
            ...
            try:
                ...
                logging.info(f"Successfully updated {media_type} for {anime_name}")
            except Exception as e:
                logging.error(f"Update callback error: {e}")
                
        elif action == "next":
            logging.info(f"Next button pressed for: {anime_name} ({media_type}, page {page})")
            ...
            try:
                ...
                logging.info(f"Successfully sent next {media_type} for {anime_name}")
            except Exception as e:
                logging.error(f"Next callback error: {e}")
...
"""
        ...
        try:
            ...
            logging.info("Successfully returned to main menu")
        except Exception as e:
    ...
    # Handle back button - return to media selection
        ...
        target = data_parts[1]
        logging.info(f"Back button pressed for: {target}")
        ...
        try:
            ...
            logging.info(f"Successfully returned to media selection for {target}")
        except Exception as e:
            logging.error(f"Back button error: {e}")
...
def log_performance_stats():
    ...
    logging.info(f"   Content cache size: {len(sent_content_ids)}")
    logging.info(f"   User offsets tracked: {len(user_offsets)}")
    logging.info(f"   Recent API requests: {len(api_request_times)}")
...
async def main():
    ...
    logging.info("🌸 Starting Shizune - Your Anime Companion...")
    
    try:
        await bot.set_my_commands(BOT_COMMANDS)
        logging.info(f"✨ Beautiful commands registered: {len(BOT_COMMANDS)} commands set")
        logging.info("💖 Shizune is ready to serve! Press Ctrl+C to stop.")
